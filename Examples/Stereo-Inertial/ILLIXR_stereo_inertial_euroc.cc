   ...
   public:
    ORB_SLAM3_ILLIXR(std::string name_, phonebook *pb_)
        ...
        std::string settings_path =
            root + "Examples/Stereo-Intertial/EuRoC.yaml";
        sys = std::make_unique<ORB_SLAM3::System>(
    ...
    void feed_imu_cam(const imu_cam_type *dataum) {
        ...
        Eigen::Vector3f &acc = datum->linear_a;
        Eigen::Vector3f &gyro = datum->angular_v;
        ORB_SLAM3::IMU::Point ORB_p(acc(0), acc(1), acc(2), gyro(0), gyro(1), gyro(2),
                              previous_timestamp);

        // Feed the IMU measurement. There should always be IMU data in each
        // call to feed_imu_cam
        assert((datum->img0.has_value() && datum->img1.has_value()) ||
               (!datum->img0.has_value() && !datum->img1.has_value()));
